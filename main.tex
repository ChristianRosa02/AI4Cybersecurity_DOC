...
ciò è cruciale per prevenire il fenomeno del \textit{data leakage} (o contaminazione dei dati): assicura infatti che il classificatore ausiliario 
...
state sovracampionate con un fattore di \textit{boost} pari a 3.0. Tale accorgimento forza la rete neurale a focalizzarsi 
maggiormente sui pattern avversariali più elusivi. Prima di introdurre la tecnica di \textit{weighted sampling}, il 
modello ausiliario tendeva a convergere rapidamente verso un'accuratezza elevata sulle classi più semplici, ma mostrava 
una performance scadente sulle perturbazioni più sofisticate. L'adozione del campionamento ponderato ha permesso di
...
effettive nel ridurre l'effetto avversario degli \textit{adversarial samples}. 

Tuttavia, queste tecniche introducono un inevitabile compromesso tra robustezza e accuratezza. La rimozione delle alte frequenze altera l'immagine e degrada spesso anche le 
...
\subsubsection{Metodologia Implementativa e Pipeline}
...
Per esplorare il delicato trade-off tra la sanitizzazione della perturbazione e 
...
\subsubsection{Metodologia di Valutazione della Compressione JPEG}
Per quantificare in modo rigoroso l'efficacia della compressione JPEG come strumento di 
...
\textbf{1. Valutazione Estesa sull'Intero Dataset:}
La prima fase prevede l'esecuzione del test sull'intero dataset di difesa indipendentemente dallo split 
(training, validation e test), 
al fine di valutare la resilienza del modello rispetto a quante più configurazioni di attacchi possibili.

Al fine di valutare correttamente  l'impatto della contromisura, il codice traccia metriche diversificate 
in base all'obiettivo dell'attacco:
\begin{itemize}
    % eliminare questa definizione se è già stata data in precedenza
    \item \textbf{Accuracy (per attacchi Untargeted):} Misura la frazione di immagini avversariali che, a seguito 
    del processo di compressione e quantizzazione DCT, vengono riclassificate correttamente secondo l'etichetta 
    originaria del soggetto.
    \item \textbf{Attack Success Rate (ASR, per attacchi Targeted Fixed):} Misura la frazione di campioni manipolati che, 
    nonostante l'applicazione della difesa, riescono ancora a costringere la rete NN1 a predire la classe bersaglio 
    predeterminata dall'attaccante (classe 2026).
    \end{itemize}

\textbf{2. Delta Quantification:}
Prima di procedere con la valutazione dei classificatori 
difesi (jpeg_classifiers), il sistema esegue una sessione di controllo sul modello privo di 
protezioni. Questo passaggio stabilisce lo zero per il calcolo dei \textit{delta} 
...
(\textit{genuine accuracy degradation}).


\textbf{Evaluation Strategy:}
A differenza dell'Adversarial Detector 
...
\begin{enumerate}
    \item \textbf{Valutazione sull'Intero Dataset:} In fase di validazione, si è scelto di trarre vantaggio 
    dall'intera disponibilità dei dati, valutando l'impatto della difesa sulla totalità del dataset. 
    ...
    dall'analisi massiva, un confronto tra due paradigmi di difesa fondamentalmente diversi richiede un 
    terreno analitico bilanciato. Per questo motivo, si è ritenuto indispensabile eseguire anche la 
    valutazione limitata al Test Set che viene poi usato anche per il classficatore binario.
\end{enumerate}

\subsubsection{Full Dataset Results}
La prima fase di valutazione della compressione JPEG ha coinvolto l'intero set di dati di difesa, 
permettendo di quantificare l'impatto del \textit{pre-processing} sulle singole famiglie di attacco. 
I risultati, riassunti nella Tabella \ref{tab:jpeg_full_dataset} e visualizzati nella Figura \ref{fig:jpeg_full_dataset}, 
mettono in luce il complesso \textit{trade-off} operativo intrinseco alle difese spaziali \textit{lossy}.
...
\textbf{Il Trade-off tra Usabilità e Robustezza:}
L'analisi del comportamento sui campioni \textbf{genuini} illustra chiaramente il costo operativo della difesa. 
Utilizzando un fattore di qualità lieve ($q=80$), l'impatto sull'accuratezza nominale del sistema è statisticamente 
nullo (+0.03\%), indicando che le feature biometriche fondamentali vengono preservate. Tuttavia, rendendo il filtro più restrittivo
($q=10$), si assiste a un crollo drammatico dell'accuratezza legittima (-13.30\%). A questo livello, 
i pesanti artefatti introdotti dalla quantizzazione DCT alterano la topologia del volto in modo irreparabile, 
rendendo il sistema inusabile per scopi di identificazione.
...
Per implementare questa valutazione, la struttura del sistema viene concettualmente riorganizzata. 
A differenza del rilevatore avversariale, che funge da vero e proprio "cancello" (o \textit{gate}) in grado di respingere 
...
legittima o manipolata da un attaccante, è obbligata ad attraversare il modulo di compressione prima di poter 
...
\begin{itemize}
    ...
    La perturbazione è stata frantumata dal filtro e la rete NN1 riesce a identificare correttamente l'utente sottostante,
    resistendo all'attacco.
    \item \textit{PassFail (Fallimento della Difesa):} La compressione non è stata abbastanza aggressiva 
    (oppure l'attacco era troppo robusto). La perturbazione sopravvive al filtro di pre-processing 
    e riesce a ingannare con successo il classificatore finale, forzando un'identità errata o un errore generico.
\end{itemize}

\vspace{10pt}

\subsubsection{Risultati sul Test Set Condiviso (\textit{Common Ground})}
...
\subsubsection{Conclusioni sull'Efficacia del Pre-Processing}
I dati empirici confermano i limiti teorici del \textit{pre-processing} contro minacce con magnitudo elevata 
(come nel nostro caso con perturbazioni $L_\infty$ pari al 10\% del range dinamico). Sebbene la compressione JPEG 
funzioni effettivamente come un filtro passa-basso disorientando i gradienti avversariali, la quantità di rumore 
iniettata dagli algoritmi è troppo massiccia. Rimuovere una quantità di segnale sufficiente a neutralizzare l'attacco 
($q=10$) comporta inevitabilmente la distruzione di una porzione critica di dettagli semantici legittimi, paralizzando 
la capacità di riconoscimento del modello NN1. Il migliore compresso risulta essere il livello $q=50$. Nonostante ciò questo
meccanismo di difesa non è adatto a garantire un meccanssmo di protezione sufficiente per un sistema di riconoscimento facciale.
...
generalizzare su perturbazioni non viste, mantenendo al contempo un'elevata efficienza computazionale. A tal fine, 
sono stati condotti due esperimenti paralleli per confrontare architetture di rete e strategie di ottimizzazione 
differenti.
...
si esplica principalmente in scenari di \textit{data poisoning}. 
...
\begin{itemize}
    ...
    La scelta è guidata dalla necessità di minimizzare l'overhead computazionale: le \textit{depthwise separable convolutions} riducono drasticamente le operazioni necessarie per 
    ...
    il \textit{compound scaling} impedisce che il segnale del rumore venga saturato o perso nei layer più profondi, garantendo 
    un'elevata sensibilità alle anomalie globali della texture dell'immagine.
\end{itemize}
Entrambi i modelli sono stati inizializzati con pesi pre-addestrati (servendosi del meccanismo di Transfer Learning) e 
dotati di un \textit{classification head} binario.
...
del rilevatore. Sebbene entrambi i modelli mostrino ottime capacità di generalizzazione, EfficientNet-B0 supera 
...
Nel nostro caso, la scelta di questo parametro definisce il livello di severità del ``gate'': una soglia più bassa 
rende il \textit{detector} molto restrittivo (poiché basta un grado minimo di anomalia per innescare il blocco), mentre 
...
\begin{itemize}
    ...
    il \textit{detector} (Falsi Negativi). Rappresenta il rischio di subire l'attacco.
...
compromesso analitico: permette al \textit{detector} di intercettare e scartare efficacemente quasi il 95\% dei 
...
Di seguito vengono riportati i risultati ottenuti sul test set per il rilevatore basato su EfficientNet-B0, utilizzando 
la soglia decisionale stabilita all'EER (0.11). Come si può osservare dalla figura \ref{fig:efficientnet_test_per_attack} entrambi i
modelli hanno performance molto simili (come confermato dalle valutazioni sul validation set), ma EfficientNet-B0 mostra 
una maggiore capacità di rilevare gli attacchi più sofisticati come CW e DeepFool confermandosi come la scelta migliore (già
fatta in precedenza sulla base dei risultati di validazione) per il modulo di rilevamento da porre a monte del sistema di face recognition.
...
\textbf{Pipeline:}
Il flusso di esecuzione dei dati all'interno del sistema difeso segue una pipeline sequenziale con logica a due stadi, implementata per 
ogni batch di immagini in ingresso:
...
\begin{enumerate}
    ...
    Il risultato finale è la predizione di identità, che viene confrontata con il label reale per valutare l'accuratezza del sistema 
    difeso.
\end{enumerate}
...
Per comprendere appieno l'efficacia del sistema difeso, non è sufficiente valutare il modulo di 
rilevamento in modo isolato; è necessario 
misurare l'impatto combinato del detector insieme al classificatore biometrico principale (NN1) 
quando operano in cascata. A questo scopo, la procedura sperimentale è stata strutturata secondo due
fasi di esecuzione distinte e sequenziali applicate sul test set:
...
avversariali, tentando poi di trasferirli per colpire il sistema reale. 

Il sistema di difesa scelta è stata quello più efficace, ovvero il detector binario basato su EfficientNet-B0.
...
Ciò conferma una solida proprietà teorica: le perturbazioni altamente ottimizzate per 
sfuggire ai filtri spaziali (e quindi al detector) tendono a compiere un 
forte \textit{overfitting} sulla geometria della rete sorgente, annullando la 
loro capacità di trasferibilità su reti dotate di distribuzioni di gradienti differenti 
(come la ResNet di NN2).
