...
\begin{itemize}
    ...
    compressione JPEG si sono dimostrate effettive nel ridurre l'effetto avversario degli adversarial samples, ma comportano 
    un compromesso tra robustezza e accuratezza, poiché spesso degradano anche le prestazioni su dati puliti. 
    \item \textbf{Detection of Adversarial Samples (Adversarial Detector):} L'obiettivo di questa difesa è rilevare durante 
    l'inferenza la presenza di perturbazioni avversarie in modo da identificare quando un input è stato manipolato. Invece 
...
\begin{itemize}
    ...
    di includere unicamente i campioni proveniente dagli attacchi targeted fixed dato che gli attacchi in modalità least likely
    ...
    perchè i sample error specific generati durante la fase di attacco hanno avuto dei risultati molto scarsi e quindi non sono
...
garantisce che nessun soggetto presente nel training set appaia nei set di validazione o di test. In un task 
biometrico come la face recognition, ciò è cruciale: assicura che il classificatore ausiliario impari a 
riconoscere il rumore avversariale, eliminando a priori il rischio che questo apprenda i tratti 
somatici, lo sfondo o l'illuminazione specifici delle identità usate in addestramento.
...
[cite: 45]. La rimozione delle alte frequenze altera l'immagine e degrada spesso anche le 
...
(le medesime condizioni vengono poi utilizzate per valutare il rilevatore binario). 
...
al fine di valutare la resilienza del modello rispetto a quante piu configurazioni di attacchi possibili.
...
nullo (+0.03\%), indicando che le feature biometriche fondamentali vengono preservate. Tuttavia, inasprendo il filtro 
...
\begin{table}[htbp]
    \centering
    \caption{Metriche complessive di accuratezza sul Test Set Condiviso per i diversi livelli di qualità JPEG e relativi delta rispetto alla configurazione indifesa.}
...
note per ottenere un necessario compromesso tra accuratezza e leggerezza computazionale, un requisito essenziale affinché 
...
la soglia decisionale stabilita all'EER (0.11). Come si puo osservare da \ref{fig:efficientnet_test_per_attack} entrambi i
